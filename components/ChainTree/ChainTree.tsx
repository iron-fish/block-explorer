...
import { Box, NAMED_COLORS, useBreakpointValue, useColorModeValue } from "@ironfish/ui-kit"
...
const ChainTree: FC<ChainTreeProps> = ({
  blocks,
  head
}) => {
  ...
  const $textColor = useColorModeValue(NAMED_COLORS.GREY, NAMED_COLORS.PALE_GREY)
  const [$selectedNode, setNode] = useState<HierarchyNode<Tree<BlockType>>>(null)
        ...
        <Group top={30} left={20}>
          {Array.from({ length: head?.sequence | 0 }).map((k, index) => (
            <text
              ...
              y={totalHeight - (head?.sequence - index) * 80 + 5 - ($selectedNode ? ($selectedNode.data.sequence > (head?.sequence - index) ? 0 : $detailsHeight) : $detailsHeight)}
              fill={$textColor}
              textAnchor="middle"
              fontSize={16}
            >
        ...
        <Group top={30} left={120}>
          ...
          {chain.descendants().map(node =>
            <Node
              ...
              node={node}
              detailsPannelHeight={$detailsHeight}
              position={calculateNodePosition(node, head, $selectedNode, $detailsHeight)}
